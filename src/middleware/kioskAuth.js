 ...
 *
 * PINs are stored as bcrypt hashes in KIOSK_STAFF_PINS (comma-separated).
...
/**
 * Hash a PIN string using bcrypt (async).
 */
 ...
 */
async function requireStaffPin(req, res, next) {
  ...
  // Compare PIN against all authorized bcrypt hashes
  let matchedHash = null;
  for (const hash of authorizedHashes) {
    try {
      const match = await bcrypt.compare(staffPin, hash);
      if (match) {
        matchedHash = hash;
        break;
      }
