...
import { CircuitBreaker, AISdkProvider } from "../src/services/aiSdkProvider";

describe("CircuitBreaker (AI SDK)", () => {
  let breaker;
...
describe("AISdkProvider", () => {
  ...
  it("returns metrics with expected shape", () => {
    const provider = new AISdkProvider();
    const metrics = provider.getMetrics();

    expect(metrics).toHaveProperty("activeProvider");
    expect(metrics).toHaveProperty("circuitBreaker");
    expect(metrics).toHaveProperty("failoverStats");
    expect(metrics.circuitBreaker).toHaveProperty("state", "CLOSED");
    expect(metrics.circuitBreaker).toHaveProperty("failureCount", 0);
  });
});
