...
| AU-11 Retention | OK | Three retention policies enforced by `src/services/retentionScheduler.js` + `src/services/retentionJob.js`: (1) **ConversationLog** — delete rows whose parent intake is REVIEWED and whose last `IntakeReview.reviewedAt` is older than `CONVERSATION_LOG_RETENTION_DAYS` (default 90). (2) **Abandoned intake** — hard-delete terminal-ABANDONED / TIMED_OUT intakes older than `INTAKE_ABANDONED_RETENTION_DAYS` (default 90) with explicit ordered child delete (no FK cascades) and `auditLogs.intakeId` nulled so 7-yr evidence survives; IN_PROGRESS intakes are NEVER auto-purged. (3) **Caseworker soft-delete** — PII columns tombstoned and `purgedAt` set after `CASEWORKER_PURGE_RETENTION_DAYS` (default 1095, floor 730 per IA-4); row preserved for AU-10 non-repudiation. All three run inside a single outer `pg_try_advisory_xact_lock` transaction with clock-skew guard, circuit breaker (5000 rows), fail-closed audit writes (`DATA_RETENTION_STARTED`/`POLICY_EXECUTED`/`COMPLETED` + per-record `DATA_RETENTION_INTAKE_PURGED` / `DATA_RETENTION_CASEWORKER_PURGED`), dry-run + leader-election defaults fail-closed. Schema migrations `20260421120000_add_intake_status_abandoned_timed_out` and `20260421120001_caseworker_purged_at`. Tests: `tests/retentionJob.test.js` (43 cases) + `tests/caseworkerPurgedAtAuth.test.js` (4 cases). |
...
| SC-28 Protection at rest | PARTIAL | **v1** — `ConversationLog.content` encrypted with AES-256-GCM, AAD = intakeId (`src/lib/fieldCrypto.js` `encryptV1`/`decryptV1`). **v2** — `Applicant.displayName` and `IntakeReview.notes` encrypted with AES-256-GCM using HKDF-derived per-(keyVersion, table, column) subkeys and AAD = `cushion-v2\|table\|column\|countyId\|rowId` — blocks cross-row, cross-column, and cross-tenant ciphertext swap. Read path: `src/lib/intakeCrypto.js` `decryptIntakeTreeInPlace` on every caseworker intake detail fetch. Failures emit `FIELD_DECRYPT_FAILED` audit events. Deferred: `HouseholdMember.displayName` (blocked on `m.displayName.toLowerCase()` compare at `src/routes/intake.js:131`), `IncomeSource.employerOrPayerName`, `Deduction.calculationNotes`. Other PII still relies on DB-level encryption (OPS) and field minimization. |
